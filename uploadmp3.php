...
///Delete old records
    ...
    /*
      if (!empty($id)) {
        if (!$data = $DB->get_record_sql("SELECT itemid FROM {files} WHERE component='mod_mediaboard' AND filearea='private' ORDER BY itemid DESC LIMIT 1", array($context->id))) { //AND contextid=?
            $fid = 1;
        } else {
            $fid = $data->itemid + 1;
        }
    ...
    if(isset($p->itemid) && is_numeric($p->itemid)) {
      ...
      $fid                    = (int)substr(time(), 2).rand(0,9) + 0;
      if ($files = $fs->get_area_files($context->id, 'mod_mediaboard', 'private', $fid)){
        $s = 2;
        $fid = rand(1, 999999999);
        while ($files = $fs->get_area_files($context->id, 'mod_mediaboard', 'private', $fid)) {
          $s = 3;
          $fid = rand(1, 999999999);
        }
      }
    ...
    echo json_encode(array("id"=>$fid, "url"=>"/pluginfile.php/".$item->contextid."/mod_mediaboard/".$id."/".$item->id."/".$item->filename, "i"=>$p->i));
    //, "text"=>mediaboard_runExternal("python /var/www/html/moodle/_py/speechtotextmp3.py {$to}")
    //echo json_encode(array("id"=>$itemid->get_id(), "url"=>(new moodle_url("/mod/mediaboard/js/recorder.swf"))));
